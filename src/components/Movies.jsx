...
const Movies = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if(response.data && response.data.success === true) {
        ...
        localStorage.setItem("lastMovies", JSON.stringify(response?.data?.results));
      }
    } catch (error) {
  ...
  return (
    <>
      <div className="w-screen min-h-screen bg-gradient-to-br from-blue-900 via-purple-700 to-pink-500 flex items-center justify-center font-poppins px-4">
        <div className="backdrop-blur-xl bg-white/10 border border-white/30 shadow-2xl rounded-3xl p-8 sm:p-10 md:p-12 w-full max-w-sm sm:max-w-md md:max-w-lg flex flex-col items-center gap-8 sm:gap-10">
          ...
          <div className="relative w-full">
            ...
            <button
              ...
              className={`absolute bottom-2 right-2 bg-gradient-to-r from-pink-500 to-purple-600 p-3 rounded-full flex items-center justify-center transition cursor-pointer mb-1 ${!prompt && "opacity-50"}`}
